...
import { emitSocketEvent } from "../socketServer";
...
const getAllChats = asyncHandler(async (req, res) => {
  ...
  const chats = await Chat.aggregate([
    {
      $match: {
        participants: { $elemMatch: { $eq: new mongoose.Types.ObjectId(req.user._id) } }, // get all chats that have logged in user as a participant
      },
    },
    {
      $sort: {
